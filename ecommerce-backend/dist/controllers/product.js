...
export const getLatestProduct = TryCatch(async (req, res, next) => {
    ...
    if (myCatch.has("latest-products"))
        products = JSON.parse(myCatch.get("latest-products"));
    else {
...
export const getAllCategories = TryCatch(async (req, res, next) => {
    ...
    if (myCatch.has("category"))
        categories = JSON.parse(myCatch.get("category"));
    else {
...
export const getAdminProducts = TryCatch(async (req, res, next) => {
    ...
    if (myCatch.has('admin-products'))
        products = JSON.parse(myCatch.get('admin-products'));
    else {
...
export const getSingleProduct = TryCatch(async (req, res, next) => {
    ...
    if (myCatch.has(`product:${id}`))
        product = JSON.parse(myCatch.get(`product:${id}`));
    else {
...
export const updateProduct = TryCatch(async (req, res, next) => {
    ...
    await product.save();
    await invalidateCatch({ product: true, productId: String(product._id), admin: true });
    return res.status(201).json({
        success: true,
