...
export const newOrder = TryCatch(async (req, res, next) => {
    const { orderItems, shippingInfo, shippingCharges, subtotal, tax, total, user, discount, } = req.body;
...
export const getMyOrders = TryCatch(async (req, res, next) => {
    ...
    let orders = [];
    if (myCatch.has(key))
        orders = JSON.parse(myCatch.get(key));
    else {
...
export const getSingleOrder = TryCatch(async (req, res, next) => {
    ...
    let order;
    if (myCatch.has(key))
        order = JSON.parse(myCatch.get(key));
    else {
