...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCatch.has(key))
        stats = JSON.parse(myCatch.get(key));
    else {
        ...
        const [thisMonthProducts, thisMonthUsers, thisMonthOrders, lastMonthProducts, lastMonthUsers, lastMonthOrders, productsCount, usersCount, allOrders, lastSixxMonthOrders, categories, femaleCount, latestTransactions] = await Promise.all([
        ...
        lastSixxMonthOrders.forEach((order) => {
        ...
        myCatch.set(key, JSON.stringify(stats));
    }
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCatch.has(key))
        charts = JSON.parse(myCatch.get(key));
    else {
        const allOrdersPromise = Order.find({}).select(["total", "discount", "subtotal", "tax", "shippingCharges"]);
        const [processingOrder, shippedOrder, deliveredOrder, categories, productsCount, productsOutOfStock, allOrders, allUsers, customerUsers, adminUsers] = await Promise.all([
        ...
        myCatch.set(key, JSON.stringify(charts));
    }
    return res.status(200).json({
        success: true,
        charts
